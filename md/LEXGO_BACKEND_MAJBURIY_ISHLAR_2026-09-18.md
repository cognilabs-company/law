...
| A9 | ✅ **PATCH TAYYOR** — PII niqoblash to'liq emas (T0-17) | `PII_PATTERNS` faqat telefon/JShShIR/pasport/email'ni yopardi — ism (F.I.Sh.) va bank karta raqami niqoblanmagan, manzil yo'q edi; unmask/restore qadami umuman yo'q edi. **Tuzatildi**: `PiiVault` klassi (bank karta + manzil regexi, qaytariladigan token, `unmask()` javobda asl qiymatni tiklaydi) + `name_terms_for(user)` + 4 ta chaqiruv nuqtasi (`main.py` chat ×2, `marketplace_routes.py` `/ai/document-analysis`, `/ai/assistant`, `/ai/cases/{id}/tools`) yangilandi. Test: `tests/test_pii_masking.py`. Diff: `LEXGO_BACKEND_FIXES_2026-09-18.patch` (repo ildizida) | Patch qo'yilishi kerak |
| A10 | **Anonim suhbat tarixi ro'yxatdan o'tgach yo'qoladi (T1-01 §4)** — backend modeli o'qib tasdiqlandi, 18.09 kechqurun | `Chat.client_id` → `clients.id` FK; mehmon uchun `client_id` = brauzerdagi tasodifiy UUID, ro'yxatdan o'tgach frontend `client_id` = `user.id` (butunlay boshqa qiymat). `ensure_client()` ikkalasini ham alohida `Client` qatori sifatida yaratadi — **hech qanday bog'lash/merge yo'q**. Kerak: `POST /auth/register/verify` (yoki alohida endpoint) ixtiyoriy `anonymous_client_id` qabul qilsin va o'sha `Client`ning barcha `Chat`larini yangi yaratilgan `user.id`ga qayta bog'lasin (yoki `Chat.client_id`ni yangilasin). Frontend tayyor: `getClientId()` (lib/client.ts) mehmon ID'ni saqlaydi — backend endpoint chiqsa, `register()`/`registerVerify()` chaqiruviga shu ID qo'shib yuborish bir qatorli o'zgarish. **Eslatma**: bu band backend'da haqiqiy `Chat`/`Client` modeliga tegishli bo'lgani uchun frontendchi tomonidan tuzatilmadi — backend dasturchi qo'lida qolishi shart | Bog'lash yo'q — ro'yxatdan o'tgan zahoti eski suhbat "yo'qoladi" (ma'lumotlar bazasida qoladi, lekin yangi akkauntga ko'rinmaydi) |
...
| A12 | ✅ **PATCH TAYYOR** — T0-11 Audit trail, 4 ta kamchilik | (a) **Bank/karta o'zgarishi loglanmagan** → `POST/DELETE /clients/me/payment-methods` endi `add_activity("payment_method_added/removed", ...)` yozadi (faqat brand+last4, to'liq raqam hech qachon). (b) **Buyurtma status yozuvi kambag'al edi** → `UserActivity`ga `device` ustuni qo'shildi (migratsiya `database.py`), `update_order_status` endi `old_status`, `ip`, `device`ni ushlab `create_order_status_history()` va `add_activity()`ga uzatadi. (c) **Eksport ruxsati alohida emas edi** → `/admin/audit-trail`da `export=csv\|json` endi mavjud `audit.export` ruxsatini talab qiladi (`require_permission`); JSON eksport ham endi `audit_export` sifatida loglanadi. (d) **Shubhali kirish faqat IP'ni ko'rar edi** → `flag_unusual_login_time()` (01:00–04:00 UTC oralig'ida, foydalanuvchining oxirgi 20 loginida bu oraliqda hech narsa bo'lmasa — `security_event` yozadi) va `flag_bulk_access()` (5 daqiqada 30+ profil ko'rish — `admin/users/{id}` endi buni tekshiradi) qo'shildi. Append-only (#3) o'zgarishsiz, robust ishlaydi. Diff: `LEXGO_BACKEND_FIXES_2026-09-18.patch` | Patch qo'yilishi va migratsiya ishga tushirilishi kerak (`device`, `requires_reaccept` ustunlari) |
| A13 | ✅ **QISMAN PATCH TAYYOR** — T0-18 §6, ikki darajali yangilanish (muhim/kichik) | `ConsentDocument`ga `requires_reaccept` ustuni qo'shildi (default=muhim, mavjud xatti-harakat saqlanadi). `POST /admin/legal/consents` (yangi versiya, `requires_reaccept` maydoni bilan) va `PATCH /admin/legal/consents/{id}` (matn tuzatish — versiya oshirmasdan, hech qachon qayta rozilik talab qilmaydi) qo'shildi — bu B1 #42 talabini ham yopadi. `GET /legal/consents/me` mantiqi tuzatildi: agar eng so'nggi faol versiya `requires_reaccept=false` bo'lsa, foydalanuvchining oldingi versiyaga bergan rozilik hali ham "qabul qilingan" hisoblanadi. **Qisman qoldi**: to'liq admin CRUD UI (frontendda ro'yxat/tahrir sahifasi) hali yo'q — faqat endpoint darajasida; 10 ta hujjatning uch tilli haqiqiy matnini PM'dan olib bazaga kiritish ham alohida ish | Patch qo'yilishi, keyin frontendga admin CRUD sahifa qo'shilishi kerak |
