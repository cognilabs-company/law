...
| A9 | **PII niqoblash to'liq emas (T0-17)** — `ai_service.py` o'qib tasdiqlandi, 18.09 kechqurun | `PII_PATTERNS` faqat telefon/JShShIR/pasport/email'ni yopadi — **ism (F.I.Sh.) va bank karta raqami umuman niqoblanmagan**, manzil ham yo'q (talab #1: "F.I.Sh., JShShIR, pasport, manzil, telefon, bank rekvizitlari"). Yana ham jiddiyi: **hech qanday unmask/restore qadami yo'q** — agar tashqi AI javobida niqoblangan qiymatni qaytarsa (masalan ismni), foydalanuvchi javobda so'zma-so'z `[PHONE_REDACTED]` kabi belgini ko'radi, haqiqiy qiymatni emas (talab #2 butunlay bajarilmagan). Tayyor patch: `ai_service.py`ga `PiiVault` klassi (bank karta + manzil regexi qo'shildi, har niqoblangan qiymat qaytariladigan token bilan almashtiriladi, `unmask()` javobda asl qiymatni tiklaydi) + `name_terms_for(user)` (login qilgan foydalanuvchining ism/familiyasi — erkin matnda ism uchun barqaror pattern yo'q, shuning uchun profildan ma'lum aniq qiymat niqoblanadi) + 4 ta chaqiruv nuqtasi (`main.py` chat ×2, `marketplace_routes.py` `/ai/document-analysis`, `/ai/assistant`, `/ai/cases/{id}/tools`) yangilandi. Avtomatik test (#5 talab) — `tests/test_pii_masking.py`, GM hujjatining o'z test matni bilan. Frontendchi tomonidan `lexgo-be` mahalliy nusxasida tayyor holda yozilgan — backend dasturchiga diff sifatida topshiriladi (git repo emas, faqat mos qo'yish uchun) | Faqat 4/6 toifa, tiklash yo'q, test yo'q |
| A10 | **Anonim suhbat tarixi ro'yxatdan o'tgach yo'qoladi (T1-01 §4)** — backend modeli o'qib tasdiqlandi, 18.09 kechqurun | `Chat.client_id` → `clients.id` FK; mehmon uchun `client_id` = brauzerdagi tasodifiy UUID, ro'yxatdan o'tgach frontend `client_id` = `user.id` (butunlay boshqa qiymat). `ensure_client()` ikkalasini ham alohida `Client` qatori sifatida yaratadi — **hech qanday bog'lash/merge yo'q**. Kerak: `POST /auth/register/verify` (yoki alohida endpoint) ixtiyoriy `anonymous_client_id` qabul qilsin va o'sha `Client`ning barcha `Chat`larini yangi yaratilgan `user.id`ga qayta bog'lasin (yoki `Chat.client_id`ni yangilasin). Frontend tayyor: `getClientId()` (lib/client.ts) mehmon ID'ni saqlaydi — backend endpoint chiqsa, `register()`/`registerVerify()` chaqiruviga shu ID qo'shib yuborish bir qatorli o'zgarish | Bog'lash yo'q — ro'yxatdan o'tgan zahoti eski suhbat "yo'qoladi" (ma'lumotlar bazasida qoladi, lekin yangi akkauntga ko'rinmaydi) |
| A11 | **Mehmon uchun "telefon tasdiqlab +N bepul savol" mexanizmi yo'q (T1-02 §4, T1-01 §3)** | GM: 3-savolda mehmonga "telefon raqamingizni tasdiqlang — 5 ta qo'shimcha savol bepul" taklifi kerak — bu FULL registratsiyadan alohida, faqat OTP bilan tasdiqlab, IP/qurilmaga bog'liq limitni oshiradigan yengil mexanizm. Hozir backendda `GuestUsage` faqat IP bo'yicha hisoblaydi, OTP-tasdiqlangan mehmon holati umuman yo'q — limit tugaganda backend ham, frontend ham to'g'ridan-to'g'ri "ro'yxatdan o'ting" deydi. Vaqtincha frontend tuzatildi: tugma endi `/login` emas `/register`ga olib boradi (mehmonda umuman akkaunt yo'q edi, "kiring" tugmasi noto'g'ri edi) va matn "telefon tasdiqlang" deb o'zgartirildi (chunki ro'yxatdan o'tishning birinchi qadami — aynan telefon tasdig'i). Agar GM talabi qat'iy bo'lsa (to'liq ro'yxatdan o'tmasdan, faqat OTP bilan +5 savol) — alohida yengil endpoint kerak | Faqat IP bo'yicha; OTP-bonus yo'q |
| A12 | **T0-11 Audit trail — 4 ta aniq kamchilik**, `marketplace_routes.py`/`models.py` o'qib tasdiqlandi, 19.09 | (a) **Bank/karta rekviziti o'zgarishi umuman loglanmaydi** — `POST/DELETE /clients/me/payment-methods` da `add_activity()` chaqiruvi yo'q (talab #1). (b) **Buyurtma status yozuvida deyarli hech narsa yo'q** — `add_activity(..., "order_status", f"Order {id} -> {status}", "", {"order_id": id})`: `ip=""` qattiq bo'sh, `meta`da faqat `order_id` — **eski holat, sabab, IP, qurilma saqlanmaydi** (talab #2; `UserActivity` modelida `device`/`user_agent` ustuni ham yo'q — qo'shilishi kerak). (c) **Eksport uchun alohida ruxsat yo'q** — `/admin/audit-trail` ko'rish va eksport bitta `users.manage` ruxsatida; JSON eksport (`?export=json`) o'zi ham jurnalga yozilmaydi (faqat CSV yoziladi) — talab #4 yarim bajarilgan. (d) **Shubhali kirish faqat IP o'zgarishini ko'radi** (`main.py:997`) — "odatiy bo'lmagan vaqt" va "ko'p yozuvni ommaviy ko'rish" uchun aniqlash yo'q (talab #5, 1/3 qoplangan). Ijobiy: append-only (#3) to'liq ishlaydi — hech qaysi endpoint `UserActivity`ni o'zgartirmaydi/o'chirmaydi; ko'pchilik harakat turi (login, rol berish, advokat tasdig'i, to'lov, workspace fayl yuklash/ko'rish/yuklab olish/o'chirish, refund) allaqachon loglanadi | (a)(b)(c)(d) yuqorida |
| A13 | **T0-18 §6 — ikki darajali yangilanish (muhim/kichik) yo'q** | GM: huquqiy hujjat yangilanganda "muhim" — keyingi kirishda qayta rozilik so'ralishi, "kichik" — faqat xabar berilishi kerak. Backendda bu ikki turni ajratadigan `requires_reaccept` (yoki shunga o'xshash) maydon umuman yo'q — butun repoda qidirildi, topilmadi. Hozir har qanday hujjat yangilanishi bir xil ishlanadi (frontend `ConsentGate` versiya farqini ko'rsa — qayta rozilik so'raydi, "kichik yangilanish — faqat xabar" varianti backendda ajrata olmaydi) | Yo'q — bitta rejim |
