...
| 69 | T0-10 §3 | `User.identity_verified_at` maydoni (hozir faqat `identity_verified`, `identity_provider`; sana faqat `identity_verification` yozuvi payload'ida) va `/auth/me` javobida `identity_verified`, `identity_provider`, `identity_verified_at` — frontend bejjni `/identity/me` orqali ko'rsatadi | Yo'q |
| 70 | T0-10 §5 | Tasdiqlanmagan advokat/yurist uchun ikki rejimli sozlama (`platform/policies` → masalan `catalog.unverified_sellers: "badge" \| "hidden"`) va `GET /lawyers` shu rejimga bo'ysunishi — hozir `/lawyers` hamma profilni qaytaradi, sozlama yo'q; `GET /lawyers/{id}` da `is_verified` bor, frontend bejj ko'rsatadi | Yo'q |
| 71 | T0-10 §4 | Advokat/yurist uchun tasdiq majburiyligi (MyID ulangach): `limited_access`/`accept_orders` ga `identity_verified` shartini qo'shish | Hozir faqat admin tasdig'i |
| 73 | Til / xatolar | Har bir `HTTPException.detail` uchun `{code, message}` (yoki `Accept-Language` bo'yicha matn) — hozir 400+ joyda faqat o'zbekcha matn; frontend eng ko'p uchraydigan 20 ta iborani ru/en'ga o'zi tarjima qiladi (`lib/apiMessage.ts`) | Yo'q |
| 74 | Katalog tillari | `LegalService.title_en`, `ServiceCategory.title_ru/title_en`, `SubscriptionPlan.name{uz,ru,en}` saqlanishi va `GET /services`, `/service-categories`, `/subscription-plans` javobida qaytishi — ru/en interfeysda kirill-o'zbek nomlar chiqmoqda | Faqat `title_uz_cyrl/latn/ru` (xizmat), kategoriya/rejalar bir tilda |
| 75 | Bildirishnomalar | `NotificationInboxOut` da `event`, `category`, `channel` ustki darajada; bitta voqea uchun kanal soni emas, bitta yozuv (yoki `event_id`); `GET /notifications?category=&read=&limit=` — hozir frontend `data.event` dan tasniflaydi va dublikatlarni o'zi yig'adi | Yo'q |
| 76 | CRM lidlar | `Lead.assigned_to_user_id` + `assigned_at` ustunlari, `POST /admin/leads/{id}/assign`, `POST /admin/leads/auto-assign {strategy}`, `GET /admin/leads?assigned_to=&region=&date_from=&date_to=`, call-markaz rollari uchun `mine` ko'rinishi — hozir frontend taqsimotni `details.assigned_to_user_id` ga yozadi (faqat leads.manage bilan) | Yo'q |
| 77 | Paket tariflar | `PATCH/DELETE /admin/subscription-plans/{id}`, `GET /admin/subscription-plans` (nofaollar bilan), `audience` uchun `client|yurist|advokat|business` qiymatlari va `sync_canonical_subscription_plans` admin qiymatini qayta yozmasligi — frontend tahrirni 404/405 da «backend kutilmoqda» deb ko'rsatadi | Faqat POST |
| 78 | ATMOS avto-to'lov | `AtmosPaymentProvider` (karta ulash + token bilan yechish), `UserSubscription.auto_renew/next_billing_at/payment_method_id`, `GET/PATCH /subscriptions/me`, oylik yechish job'i, `subscription_renewed/failed` bildirishnomalari — frontend tumbler va karta kartasi tayyor (sozlama hozircha qurilmada) | Yo'q |
| 79 | Dashboard filtrlari | `GET /admin/dashboard`, `/analytics/ceo`, `/calls/analytics`, `/retention/overview`, `/quality/overview`, `/lawyers/me/stats` uchun `region`, `date_from`, `date_to` parametrlari va `by_region` bo'linmalar — frontend parametrlarni yuboradi va imkon boricha o'zi filtrlaydi; ma'lumot bo'lmasa demo ko'rsatadi | Yo'q |
| 80 | Xizmatlar admin | `PATCH/DELETE /admin/service-categories/{id}`, `GET /admin/services?include_inactive=true`, `PATCH /admin/services/{id}` da metadata yozuvi bo'lmasa yaratish, PATCH uchun Pydantic sxema (422) | Yo'q |
| 81 | Uchrashuv yozib olish | `call_participant` da `recording_consent {requested_by, approved_by, denied_by, at}` va call socket allowlist'ga `recording.*` voqealari — hozir rozilik LiveKit data-kanalida (saqlanmaydi); `call.incoming` da `caller_name` | Yo'q |

> 18.09 (kechki): #77 va #80 kelguncha admin tarif/kategoriya tahriri va o'chirish **platforma siyosati overlay** orqali ishlaydi — `PUT /admin/platform/policies/payment` → `plans.overrides[slug]` (title/narx/audience/benefits/is_active/hidden) va `.../order` → `catalog.categories[id]` (title/hidden). Frontend avval real PATCH/DELETE ni sinaydi (404/405 → overlay). Hujjatda bo'lmagan `lexgo-ai-yurist-advokat-seller`, `lexgo-ai-jismoniy-shaxs`, `biznes-abonent-*` rejalari sukut bo'yicha yashirin (`lib/catalogOverlay.ts`). Backend PATCH/DELETE chiqqach overlay avtomatik chetlab o'tiladi; `sync_canonical_subscription_plans` admin tahririni qayta yozmasligi kerak (#77).
